...
beta1 <- c(1,1, rep(0, p-1))
eta1 <-exp(Xmat %*% beta1)
prob<- (eta1)/(1+eta1)
B <- rbinom(n.size, size = 1, prob = prob)
test_data <- data.frame(X,y = B)
...
ggplot(df_combined, aes(x = method, y = x, group = d, color = d)) +
  ...
  labs(x = "Sample size", y = "Distance to CS (log scale)", title = "Example 1") +
  ...
  scale_color_discrete(name = "Model", breaks = c("LLO(lambda = 0)", "LLO(lambda > 0)", "SAVE", "PHD", "POTD"),                           
                       labels = c(expression(paste("LLO(", lambda, " = 0)")), expression(paste("LLO(", lambda, " > 0)")), "SAVE", "PHD", "POTD"))
...
ggplot(df_MC, aes(x = method, y = x, group = d, color = d, linetype = d)) +
  ...
  geom_line(position = position_dodge(width = 0.1), size = 0.5) +
  labs(x = "Sample size", y = "Mislassification risk", title = "Example 1") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "top",
