...
response_I <- generate_binary_response("I", X)
...
save(df_combined , file = "space_dist_ex2.Rdata")
...
ggplot(df_combined, aes(x = method, y = x, group = d, color = d)) +
  ...
  labs(x = "Sample size", y = "Distance to CS (log scale)", title = "") +
...
ggplot(df_MC, aes(x = method, y = x, group = d, color = d, linetype = d)) +
  ...
  geom_line(position = position_dodge(width = 0.1), size = 0.5) +
  labs(x = "Sample size", y = "Mislassification risk", title = "Example 2") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "top",
...
# 
# setwd("D:/PostDoc work/Figures/classification/Simulation/code/Ex2")
# save(df_AM, file = "AM_ex2.Rdata")
# # Plot
# 
# # Plot
# 
